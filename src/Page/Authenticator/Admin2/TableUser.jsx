...
export default function TableUser() {
  ...
  const unblockById = () => {
    ...
    unblockData("user", productRequestDefault.id)
      ...
      .catch((error) => {
        ...
        api["error"]({
          message: textApp.TableProduct.Notification.deleteError.message,
          description: "Không thể mở khóa tài khoản này",
        });
      });
    setDataRun(!dataRun);
  };
  ...
  const LoginRequestDefault = {
    ...
    const UpdateUserMessenger = yup.object({
    ...
    const methodUpdateUser = useForm({
      resolver: yupResolver(UpdateUserMessenger),
      defaultValues: {
        name: "",
        phone: "",
        email: "",
      ...
      useEffect(() => {
        if (productUpdate) {
          methodUpdateUser.reset({
            name: productUpdate.name,
            phone: productUpdate.phone,
            email: productUpdate.email,
          });
        }
      }, [productUpdate, methodUpdateUser]);
  ...
  return (
    <>
      ...
      >
        <FormProvider {...methodUpdateUser}>
          <form className="flex flex-col gap-6" onSubmit={onSubmit1}>
            <ComInput
              label={"Tên tài khoản"}
