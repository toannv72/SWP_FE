    ...
    try {
      if (checked === 1) {
        const response = await getData(
          `/artwork/search?name=${search}&page=${pageNumber}&limit=20`
        );
        const newArray =
      ...
      if (checked === 3) {
        const response = await getData(
          `/user/search?name=${search}&page=${pageNumber}&limit=20`
        );
  ...
  return (
    <>
      ...
      <div className="flex gap-10">
        <div
          className=" items-center mb-2 pl-20 pt-10 "
        ...
        <div className="p-4 flex items-center">
          {products?.length !== 0 ? (
            ...
            <div className=" text-center w-screen">
              {" "}
              Không tìm thấy bài viết đang tìm kiếm
            </div>
          )}
        </div>
      </div>
